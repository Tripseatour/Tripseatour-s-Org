...
import { Lock, ShieldCheck, AlertCircle, ArrowRight, X, Mail, CheckCircle2, KeyRound, Sparkles, UserCheck, ExternalLink } from 'lucide-react';
    ...
    if (typeof window !== 'undefined' && (window as any).google?.accounts?.oauth2) {
      try {
        const client = (window as any).google.accounts.oauth2.initTokenClient({
          client_id: '842635042455-dev.apps.googleusercontent.com',
            ...
            if (tokenResponse && tokenResponse.access_token) {
              try {
                ...
                if (profile && profile.email) {
                  ...
                  verifyAndLogin(authorizedEmails[0] || 'asmr9941@gmail.com');
                }
              } catch (err) {
                verifyAndLogin(authorizedEmails[0] || 'asmr9941@gmail.com');
              }
          ...
          {authMode === 'gmail' ? (
            <div className="space-y-4">
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-white hover:bg-slate-50 active:scale-[0.99] text-slate-800 font-extrabold py-3.5 px-4 rounded-2xl border-2 border-slate-300 text-xs transition shadow-sm flex items-center justify-center gap-3 disabled:opacity-50"
              >
                ...
                <span>{isLoading ? 'กำลังเข้าสู่ระบบ Google...' : 'เข้าสู่ระบบด้วย Google Account (OAuth 2.0)'}</span>
              </button>
