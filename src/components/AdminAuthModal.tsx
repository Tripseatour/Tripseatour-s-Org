...
import { Lock, ShieldCheck, AlertCircle, ArrowRight, X, CheckCircle2, KeyRound } from 'lucide-react';
...
export const AdminAuthModal: React.FC<AdminAuthModalProps> = ({
  ...
  const handleRealGoogleOAuth = () => {
    ...
    // Real client ID from project config
      ...
      try {
        const client = (window as any).google.accounts.oauth2.initTokenClient({
            ...
            if (tokenResponse && tokenResponse.access_token) {
              try {
                ...
                if (profile && profile.email) {
                  ...
                  setErrorMsg('ไม่สามารถดึงข้อมูลอีเมลจาก Google ได้');
                  setIsLoading(false);
                }
              } catch (err) {
                setErrorMsg('เกิดข้อผิดพลาดในการดึงข้อมูลโปรไฟล์ Google');
                setIsLoading(false);
              }
            } else {
              setIsLoading(false);
          ...
          error_callback: (error: any) => {
            ...
            setIsLoading(false);
            setErrorMsg('เกิดข้อผิดพลาดในการเชื่อมต่อ Google OAuth (กรุณาตรวจสอบว่าลงทะเบียน Authorized Origin ใน Google Cloud Console แล้วหรือใช้รหัส PIN)');
          }
        });
        client.requestAccessToken({ prompt: 'select_account' });
      } catch (e) {
        setIsLoading(false);
        setErrorMsg('ไม่สามารถเปิดป๊อปอัป Google Account Chooser ได้');
      }
    } else {
      setIsLoading(false);
      setErrorMsg('Google Sign-In SDK ยังไม่พร้อมใช้งานในระบบขณะนี้');
    }
  };
          ...
          {authMode === 'gmail' ? (
            <div className="space-y-4 py-2">
              <p className="text-xs text-slate-600 text-center leading-relaxed">
                คลิกปุ่มด้านล่างเพื่อเข้าสู่ระบบผู้ดูแลด้วยบัญชี <strong className="text-slate-800">Google Account</strong>
              </p>

              {/* Primary Google OAuth Chooser Button */}
              <button
                type="button"
                onClick={handleRealGoogleOAuth}
                disabled={isLoading}
                className="w-full bg-white hover:bg-slate-50 active:scale-[0.99] text-slate-800 font-extrabold py-3.5 px-4 rounded-2xl border-2 border-slate-300 text-xs transition shadow-sm flex items-center justify-center gap-3 disabled:opacity-50 cursor-pointer"
              >
                ...
                <span>{isLoading ? 'กำลังเปิด Google Account Chooser...' : 'เข้าสู่ระบบด้วย Google Account (OAuth 2.0)'}</span>
              </button>
